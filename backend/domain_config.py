...
SupportedDomain = Literal["hiring", "lending", "healthcare", "custom"]
...
class DomainConfig(BaseModel):
    ...
    @field_validator("protected_attributes", "feature_columns", "required_columns", mode="before")
    @classmethod
    def normalize_list_columns(cls, value: Any) -> list[str]:
        ...
        if isinstance(value, str):
            ...
            return [_normalize_column_name(token) for token in tokens]
        if isinstance(value, list):
            return [_normalize_column_name(item) for item in value if str(item).strip()]
        raise ValueError("Expected list of column names.")
    ...
    def normalize_column_map(cls, value: Any) -> dict[str, str]:
        ...
        for key, mapped in value.items():
            ...
            normalized_value = _normalize_column_name(mapped)
...
def _preset_templates() -> dict[str, DomainConfig]:
    return {
        ...
        "lending": DomainConfig(
            ...
            feature_columns=["income", "credit_score", "loan_amount", "employment_years", "debt_to_income"],
        ...
        "healthcare": DomainConfig(
            ...
            feature_columns=["severity_score", "wait_time_hours", "prior_visits", "distance_km"],
            display_name="Medical Admission",
...
def list_domain_templates() -> list[DomainConfig]:
    return [template.model_copy(deep=True) for template in PRESET_DOMAIN_TEMPLATES.values()]


def detect_domain(columns: list[str]) -> DomainConfig | None:
    normalized = {_normalize_column_name(column) for column in columns}
    for template in list_domain_templates():
...
def parse_domain_config_payload(
    ...
    normalized_columns = [_normalize_column_name(column) for column in csv_columns]
    ...
    requested_domain = _normalize_column_name(payload_data.get("domain") or fallback_domain or "")
    ...
    if requested_domain in PRESET_DOMAIN_TEMPLATES:
        ...
        for key in ("outcome_column", "outcome_positive_value", "protected_attributes", "feature_columns", "column_map"):
    ...
    if requested_domain != "custom":
        supported = ", ".join(["hiring", "lending", "healthcare", "custom"])
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"Unsupported domain '{requested_domain}'. Supported values: {supported}.",
        )
...
def validate_required_columns(config: DomainConfig, csv_columns: list[str]) -> tuple[list[str], list[str]]:
    normalized_columns = [_normalize_column_name(column) for column in csv_columns]
